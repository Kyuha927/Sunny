...
aliases:
  ...
  - 에이전트 아키텍처
date: 2026-03-22
summary: "이 문서는 차세대 에이전트 시스템 구축을 위한 로드맵으로, 무오염 인계, 하이브리드 자동화, 다중 에이전트 오케스트레이션 등 핵심 발전 방향을 제시한다. 또한 웹 자동화 및 멀티에이전트 프레임워크 스택과 개발 노트를 추적하며 실제 구현 전략을 다룬다."
type: project
---
# 🤖 [[Advanced Agent Systems|Advanced Agent Systems]] (차세대 에이전트 시스템 로드맵)
...
1. **[[Contamination-Free Handoff|Contamination-Free Handoff]] (무오염 인계)**: 에이전트가 단일 세션에서 긴 문맥을 유지하며 삽질하는 것을 막고, 가장 작은 다음 단계(Smallest Next Step)를 도출한 뒤 깨끗한 환경(Fresh Session)으로 작업을 이관하는 시스템 확립 (Codex 모델 참고).
2. **[[Hybrid Automation|Hybrid Automation]] (하이브리드 자동화)**: [[LLM|LLM]]의 추론 병목을 순수 코드로 대체. `[[Browser-use|browser-use]]` 기반의 빠른 DOM 액션과 `PydanticAI/[[TheAgenticBrowser|TheAgenticBrowser]]` 기반의 에러 검수(Critique) 파트를 결합하여 속도와 안정성의 시너지를 창출.
3. **[[Multi-Agent Orchestration|Multi-Agent Orchestration]]**: 역할을 쪼갠 다중 에이전트(`docs_researcher`, `worker`, `critique` 등)가 백그라운드에서 병렬 동작할 수 있도록 권한과 통신 프로토콜 정립.
...
|---|---|---|
| **[[CrewAI|CrewAI]]** | 역할(Role) 기반 자율 위임 + `Memory` 시스템(세션 간 기억 유지) + LangChain 제거로 독립 경량화 + **배포 속도 [[LangGraph|LangGraph]] 대비 40% 빠름** | 미연시 자동화에서 `프롬프트 작성자 → 영상 생성자 → 검수자` 역할 분리에 적합 |
...
| **[[AutoGen|AutoGen]] (MS)** | `Writer ↔ Critic` 자기 반성(Self-Reflection) 루프: 에이전트가 자기 산출물을 자체 검증 후 수정하는 반복 + 비동기 이벤트 드리븐 아키텍처 | 영상 프롬프트 품질을 올리는 자동 정제(Refinement) 파이프라인에 결합 가능 |
| **[[MetaGPT|MetaGPT]]** | 소프트웨어 회사 시뮬레이션(PM → 아키텍트 → 엔지니어) + **SOP(표준운영절차)를 프롬프트로 인코딩** → 에이전트 간 구조화된 소통 + 한 줄 요구사항으로 전체 프로젝트 산출 | 픽셀 에이전트 같은 대규모 코드베이스 리팩토링을 자동 기획/분배하는 데 참고할 아키텍처 |

### 인프라 & CLI
- **[[Codex CLI|Codex CLI]]**: 터미널 기반 다이렉트 에이전트, 백그라운드 병렬 파일 서치 및 시스템 Handoff 구조의 롤모델.
- **[[Claude Code CLI|Claude Code CLI]]**: 고도화된 컨텍스트 조망 능력과 대규모 코드베이스 리팩토링 특화. Codex CLI와 상호 보완적인 강력한 3각 편대(GUI-Codex-Claude Code) 구성의 핵심 아키텍트.
